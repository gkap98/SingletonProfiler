...
// START TIME AND END TIME
        // String html = "";
        // html += "<html><h1>Profiler Time Log</h1> <table><tr><th>Profiler
        // ID</th><th>Start Time</th><th>End Time</th><th>Start Message</th><th>End
        // Message</th></tr>";
...
// TIME DURATION RATHER THAN START STOP
        ...
        for (String id : profiles.keySet()) {
            for (int i = 0; i < profiles.get(id).size(); i++) {
                html += "<tr>";
                html += "<td>" + id + "</td>";
                html += "<td>" + profiles.get(id).get(i).differenceNS(profiles.get(id).get(i).getBegin(),
                        profiles.get(id).get(i).getEnd()) + "</td>";
                html += "<td>" + profiles.get(id).get(i).getBeginMessage() + "</td>";
                html += "<td>" + profiles.get(id).get(i).getEndMessage() + "</td>";
                html += "</tr>";
            }
